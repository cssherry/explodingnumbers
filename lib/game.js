(function(){
  ...
  Game.prototype.addFallingNumbers = function(){
    ...
    do {
      if(this.addFallingNumberTries > 30) throw "screenIsFull"; // Throw error if all the positions are full
  ...
  Game.prototype.registerClickedNumber = function (pos) {
    ...
    if (!clickedNumber) {
      return;
    } else if ( answerIndex !== -1) { //Unselect number
  ...
  Game.prototype.numberInSurroundingArea = function (pos) {
    ...
    console.log(this.numbersHeight);
    for (var i = 0; i < this.numbersHeight / 2; i++) {
      for (var j = 0; j < this.numbersHeight / 2; j++) {
        var testPos1 = [pos[0] + i, pos[1] - j];
        var testPos2 = [pos[0] - i, pos[1] - j];
        var testPos3 = [pos[0] + i, pos[1] + j];
        var testPos4 = [pos[0] - i, pos[1] + j];
        ...
        } else if (this.fallingNumbers[testPos3]) {
          number = this.fallingNumbers[testPos3];
        } else if (this.numbers[testPos3]) {
          number = this.numbers[testPos3];
        }  else if (this.fallingNumbers[testPos4]) {
          number = this.fallingNumbers[testPos4];
        } else if (this.numbers[testPos4]) {
          number = this.numbers[testPos4];
        }
      }
    }
    return number;
