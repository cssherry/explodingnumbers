(function(){
  ...
  Game.prototype.addFallingNumbers = function(){
    ...
    this.fallingNumbers.push(newNumber.fabricObject);
  ...
  Game.prototype.collidesWithOtherNumber = function (pos, currentFallingPosition) {
    ...
    for (var j = 0; j < this.fallingNumbers.length; j++) {
      ...
      if (j !== currentFallingPosition) {
        // Sometimes the group overlaps, even though objects don't. Compare only rectangles
        if (newNumber.fabricObject.intersectsWithObject(this.fallingNumbers[j]) && this.fallingNumbers[j].intersectsWithObject(newNumber.fabricObject)) {
          return true;
        }
      }
    }
  ...
  Game.prototype.generateNewEquation = function () {
    return new ExplodingNumbers.Equation(this.difficulty, this.numbersOnScreen, this.DIM_X / 2 - 60);
  };
