(function(){
    ...
    for (var j = 0; j < this.fallingNumbers.length; j++) {
      ...
      if (bottomRowItem) {
        ...
        if (this.bottomRow[number.pos[0]]) {
          this.bottomRow[number.pos[0]].push(number.fabricObject);
        } else {
          this.bottomRow[number.pos[0]] = [number.fabricObject];
        }
  ...
  Game.prototype.generateNewEquation = function () {
    return new ExplodingNumbers.Equation(this.difficulty, this.numbersOnScreen, this.DIM_X / 2 - 10);
  };
  ...
  Game.prototype.moveNumbersDown = function (xPosition, number) {
    ...
    for (var i = 0; i < column.length; i++) {
      column[i].fabricObject.set({top: this.bottomYPosition - (this.numbersHeight * i)});
    }
  };
})();
