(function(){
  ...
  var Game = ExplodingNumbers.Game = function(dimX, dimY, numColumns){
    ...
    this.bottomYPosition = Math.floor(this.DIM_Y - this.numbersHeight); //Define this here to be used when making initial row and assigning floor numbers
    ...
    this.equation = new ExplodingNumbers.Equation(this.difficulty, this.numbersOnScreen);
  };
  ...
  Game.prototype.addInitialNumbers = function(){
    ...
    for (var i = 0; i < this.numColumns; i++) {
      ...
      var row = new fabric.Group([ number ], { left: xPosition, top: this.bottomYPosition });

      this.fixedNumbersTopLayer.push(row);
  ...
  Game.prototype.collidesWithOtherNumber = function (pos) {
    this.newNumber = new ExplodingNumbers.Number([randomX, Math.floor(this.screenOffsetY)], this.numbersHeight, this.difficulty);

    for (var i = 0; i < this.bottomRow.length; i++) {
      if (this.newNumber.intersectsWithObject(this.bottomRow[i])) {
        return true;
      }
    }

    for (var j = 0; j < this.fallingNumbers.length; j++) {
      if (this.newNumber.intersectsWithObject(this.fallingNumbers[j])) {
        return true;
      }
    }
  ...
  Game.prototype.moveObjects = function() {
    for (var j = 0; j < this.fallingNumbers.length; j++) {
      ...
      var bottomRowItem = this.bottomRow(newPosition);
  ...
  Game.prototype.bottomRow = function (pos) {
    var testNumber = new ExplodingNumbers.Number([randomX, Math.floor(this.screenOffsetY)], this.numbersHeight, this.difficulty);

    for (var i = 0; i < this.bottomRow.length; i++) {
      if (testNumber.intersectsWithObject(this.bottomRow[i]) || this.bottomYPosition < pos[1]) {
