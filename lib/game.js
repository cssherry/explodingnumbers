(function(){
  ...
  Game.prototype.registerClickedNumber = function (pos) {
    var clickedNumber = this.numbers[pos] || this.fallingNumbers[pos];
    var answerIndex = this.playersAnswer.indexOf(clickedNumber.content);
    if ( answerIndex !== -1) {
      this.playersAnswer.splice(answerIndex, 1);
    } else {
      this.playersAnswer.push(clickedNumber.content);
    }
    
    if (this.correctAnswer(this.playersAnswer.join)) {
      return true;
    } else {
      return false;
    }
